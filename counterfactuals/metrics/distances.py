...
def continuous_distance(
    ...
    agg_funcs = {"mean": np.mean, "max": np.max,
                 "min": np.min, "no": lambda x: x}
    ...
    assert agg in agg_funcs.keys(
    ), f"Param agg should be one of: {agg_funcs.keys()}"
    assert (
    ...
    dist = cdist(
        X_test[:, continuous_features], X_cf[:,
                                             continuous_features], metric=metric
    ).diagonal()
...
def categorical_distance(
    ...
    dist = cdist(
        X_test[:, categorical_features], X_cf[:,
...
def distance_combined(
    ...
    ratio_continuous = (
        ...
        number_features if ratio_cont is None else ratio_cont
    )
