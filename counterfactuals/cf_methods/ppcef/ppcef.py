...
class PPCEF(BaseCounterfactual):
    ...
    def _search_step(
        ...
        if log_prob_threshold is None:
            raise ValueError(
                "Parameter 'log_prob_threshold' should be in kwargs")
        ...
        disc_logits = (
            disc_logits.reshape(
                -1) if disc_logits.shape[0] == 1 else disc_logits
        )
        context_target = (
        ...
        loss_disc = self.disc_model_criterion(
            disc_logits, context_target.float())
    ...
    def explain_dataloader(
        ...
        **search_step_kwargs,
        ...
        for xs_origin, contexts_origin in dataloader:
            ...
            contexts_target = (
                torch.abs(1 - contexts_origin)
                if target_class_value is None
                else target_class_value * torch.ones(xs_origin.shape[0], 1)
            )
            ...
            for _ in (epoch_pbar := tqdm(range(epochs))):
                ...
                disc_loss = loss_components["loss_disc"].detach(
                ).cpu().mean().item()
                prob_loss = loss_components["max_inner"].detach(
                ).cpu().mean().item()
        ...
        X_orig = np.concatenate(original, axis=0)
        X_cf = X_orig + deltas
        return (
            X_cf,
            X_orig,
