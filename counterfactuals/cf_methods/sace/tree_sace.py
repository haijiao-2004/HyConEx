...
class TreeSACE(SACE):
    ...
    def get_counterfactuals(
        ...
        if self.use_instance_weights:
            sample_weight = self.__calculate_weights__(
                nx, self.nX, self.metric)
        else:
            sample_weight = None
    ...
    def __get_counterfactuals(
        ...
        if not self.pooler:
            ...
            node_index_path_x, leave_id_x = self.__get_x_infos(
                self.pooler.transform(x))
            feature_lower_upper_x = self.__get_feature_boundaries(
        ...
        for cf_idx, l_idx in zip(range(len(X_cfcm)), np.unique(leave_id_m)):
            ...
            if not nvf_changed:
                ...
                score = alpha * dist + (1 - alpha) * \
                    1 / (len(X_cfc_l) / len(X_cfc))

                # cfc_o = x.copy()
                cfc_o = x.copy() if not self.pooler else self.pooler.transform(x)
                if self.closest_in_leaf:
                    cfl_idx = np.argmin(
                        self.cdist(nx, nX_cfc_l, metric=self.metric,
                                   w=self.weights)
                    )
    ...
    def __get_prototypes(
        ...
        for pr_idx, l_idx in zip(range(len(X_prcm)), np.unique(leave_id_m)):
            ...
            if not nvf_changed:
                ...
                score = alpha * dist + (1 - alpha) * \
                    1 / (len(X_prc_l) / len(X_prc))

                if self.closest_in_leaf:
                    cfl_idx = np.argmin(
                        self.cdist(nx, nX_prc_l, metric=self.metric,
                                   w=self.weights)
                    )
    ...
    def __get_feature_boundaries(self, x, node_index_path):
        ...
        for node_id in node_index_path:
            ...
            if f == -2:
                continue
            threshold_sign = "<=" if x[0,
                                       f] <= self.threshold[node_id] else ">"
