...
class SACE(ABC):
    ...
    def __init_cont_cat_features(self, continuous_features):
        if isinstance(self.metric, str):
            self.continuous_features = (
                # np.arange(self.nbr_variable_features).tolist()
                continuous_features
            )
        ...
        if self.categorical_features_index_lists:
            self.nbr_cate_features_real = len(
                self.categorical_features_index_lists)
        else:
    ...
    def _calculate_prototype_score(self, x, pr, beta=0.5):
        ...
        if d == 0.0:
            return np.inf
        l = cdist(self._predict_proba(x), self._predict_proba(pr), metric="cosine")[
            0
        ]  # noqa: E741
        if l == 0.0:
            return np.inf
